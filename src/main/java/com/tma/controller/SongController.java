...
public class SongController {
	...
	@ApiOperation(value = "List all songs", notes = "List all people using paging", response = Song.class, responseContainer = "List")
	@ResponseBody
	public Page<Song> getAll(@RequestParam String page,
			@RequestParam String size) {
		return songService.findAll(new PageRequest(Integer.parseInt(page),
				Integer.parseInt(size), Direction.ASC, "name"));
	}

	@RequestMapping(value = "/search", method = RequestMethod.GET)
	@ResponseBody
	public Page<Song> getByName(@RequestParam String name,
			@RequestParam String page, @RequestParam String size) {
		return songService.findByName(name,
				new PageRequest(Integer.parseInt(page), Integer.parseInt(size),
						Direction.ASC, "name"));
	}
	...
	@ResponseBody
	public Song getWithId(
			@ApiParam(value = "ID of song to fetch", required = true) @PathVariable String id) {
